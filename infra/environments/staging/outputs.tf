# Every output below maps directly to a GitHub Environment variable/secret
# named in .github/workflows/{web,backend}-staging.yml's setup checklists,
# and to REPLACE_* placeholders in .aws/task-definition.*.staging.json.
# After `terraform apply`, run `terraform output` and copy each value into
...
output "secrets_manager_arn" {
  ...
  description = "-> replaces every REPLACE_WITH_STAGING_SECRETS_MANAGER_ARN in .aws/task-definition.*.staging.json"
}

output "ecs_task_execution_role_arn" {
  value       = module.ecs_cluster.task_execution_role_arn
  description = "-> replaces REPLACE_WITH_ECS_TASK_EXECUTION_ROLE_ARN in .aws/task-definition.*.staging.json"
}

output "ecs_task_role_arn" {
  value       = module.ecs_cluster.task_role_arn
  description = "-> replaces REPLACE_WITH_ECS_TASK_ROLE_ARN in .aws/task-definition.*.staging.json"
}
